...
import AvisoLegal from "../views/AvisoLegal"; // import absoluto
    ...
    <>
      ...
      >
        <div className="max-w-5xl mx-auto px-6 relative z-10">
          <motion.div className="grid grid-cols-1 md:grid-cols-3 gap-12 lg:gap-16 mb-16 text-center md:text-left" variants={staggerVariants}>
            {/* Brand Section */}
            <motion.div className="flex flex-col items-center md:items-start gap-6" custom={0} variants={itemVariants}>
            ...
            {/* Información */}
            <motion.div className="flex flex-col items-center md:items-start gap-6" custom={1} variants={itemVariants}>
            ...
            {/* Social & Contact */}
            <motion.div className="flex flex-col items-center md:items-start gap-6" custom={2} variants={itemVariants}>
              ...
              <motion.div className="flex gap-4 justify-center md:justify-start" variants={staggerVariants}>
                {[{ icon: FaFacebook, href: "https://www.facebook.com/asirtec/", color: "hover:text-blue-400" },
                  ...
                  { icon: FaInstagram, href: "https://www.instagram.com/asirtec/?hl=es", color: "hover:text-pink-400" }].map((SocialIcon) => (
                  <motion.a
                    key={SocialIcon.href}
          ...
          {/* Bottom Section */}
          <motion.div className="border-t border-gray-700/50 pt-8 text-center" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ delay: 0.6, duration: 0.8 }} viewport={{ once: true }}>
            <motion.p className="text-gray-400 text-sm mb-2">&copy; {new Date().getFullYear()} Zaitec Innova. Todos los derechos reservados.</motion.p>
            <motion.p className="text-gray-500 text-xs">Transformando ideas en soluciones digitales</motion.p>
          </motion.div>
        </div>
      ...
      {/* Aviso Legal renderizado dinámicamente */}
      {showAviso && (
        <div className="fixed inset-0 bg-black/50 z-50 overflow-auto">
          <div className="bg-white max-w-4xl mx-auto my-20 p-8 rounded-xl relative">
            <button
              onClick={() => setShowAviso(false)}
              className="absolute top-4 right-4 text-gray-700 font-bold text-xl hover:text-black"
            >
              ×
            </button>
            <AvisoLegal />
